...
import { sharpmakerElevation } from "../utils/math";

export class VsaElevationCalculator extends LitElement {
  static styles = css`
    :host {
      display: block;
    }
  ...
  @property({ type: Number }) desiredAngle: number | null = null;
  @property({ type: Number }) sharpenerAngle: number | null = null;
  @property({ type: Boolean, state: true }) private _shouldFocus = false;

  private _onDesiredAngle(e: Event) {
    const input = e.target as HTMLInputElement;
    this.desiredAngle = input.value ? Number(input.value) : null;
    this._shouldFocus = false; // Don't auto-focus when user is actively typing
  }

  private _onSharpenerAngle(e: Event) {
    const select = e.target as any;
    this.sharpenerAngle = select.value ? Number(select.value) : null;
    this._shouldFocus = false; // Don't auto-focus when user is actively selecting
  }

  render() {
    const elev = sharpmakerElevation(
      this.desiredAngle ?? undefined,
      this.sharpenerAngle ?? undefined
    );
      ...
      <section class="calc">
        ...
        <div class="input-row">
          <sl-input
            ...
            type="number"
            step="0.1"
            .value=${this.desiredAngle?.toString() || ""}
          ...
          <sl-select
            ...
            .value=${this.sharpenerAngle?.toString() || ""}
...
customElements.define("vsa-elevation-calculator", VsaElevationCalculator);
